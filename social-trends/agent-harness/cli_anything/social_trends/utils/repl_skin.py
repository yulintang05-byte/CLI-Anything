...
Social-media-first branding with gradient-style accent colors.
"""
...
# Social-media accent: TikTok-inspired hot pink / coral
_ACCENT = "\033[38;5;205m"
_ACCENT2 = "\033[38;5;39m"   # Instagram blue
...
_ICON = f"{_MAGENTA}{_BOLD}◆{_RESET}"
_ICON_SMALL = f"{_MAGENTA}▸{_RESET}"
...
class ReplSkin:
    """REPL skin for the Social Trends CLI harness."""
    ...
    def print_banner(self):
        ...
        icon = self._c(_MAGENTA + _BOLD, "◆")
        brand = self._c(_CYAN + _BOLD, "cli-anything")
        dot = self._c(_DARK_GRAY, "·")
        name = self._c(_ACCENT + _BOLD, "Social Trends")
        title = f" {icon}  {brand} {dot} {name}"

        ver = f" {self._c(_DARK_GRAY, f'   v{self.version}')}"
        tip1 = f" {self._c(_DARK_GRAY, '   Scrape trends · Optimize accounts · Build theme pages')}"
        tip2 = f" {self._c(_DARK_GRAY, '   Type help for commands, quit to exit')}"
        empty = ""
        ...
        print(_box_line(tip1))
        print(_box_line(tip2))
        print(bot)
        print()
    ...
    def prompt(self, context: str = "", modified: bool = False) -> str:
        parts = []
        if self._color:
            parts.append(f"{_MAGENTA}◆{_RESET} ")
        else:
            parts.append("> ")

        parts.append(self._c(_ACCENT + _BOLD, "social-trends"))
    ...
    def get_prompt_style(self):
        ...
        return Style.from_dict({
            "icon": "#d75faf bold",
            "software": "#ff5faf bold",
            "bracket": "#585858",
            "context": "#bcbcbc",
            "arrow": "#808080",
            "completion-menu.completion": "bg:#303030 #bcbcbc",
            "completion-menu.completion.current": "bg:#d75faf #000000",
            "auto-suggest": "#585858",
        })
    ...
    def trend(self, message: str):
        print(f"  {self._c(_MAGENTA + _BOLD, '▲')} {self._c(_ACCENT, message)}")
    ...
    def table(self, headers: list, rows: list, max_col_width: int = 45):
        if not headers:
            return
        ...
        header_cells = [
            self._c(_MAGENTA + _BOLD, pad(h, col_widths[i]))
    ...
    def print_goodbye(self):
        print(f"\n  {_ICON_SMALL} {self._c(_GRAY, 'Stay on trend. Goodbye!')}\n")
