"""cli-anything REPL Skin for Social Trends — hot pink/purple accent."""
...
# Social-trends accent: TikTok-style hot pink
...
class ReplSkin:
    ...
    def prompt_tokens(self, context: str = "") -> list:
        tokens = [("class:icon", "◆ "), ("class:software", "social-trends")]
        if context:
            tokens += [("class:bracket", " ["), ("class:context", context), ("class:bracket", "]")]
        tokens.append(("class:arrow", " ❯ "))
        return tokens

    def prompt(self, context: str = "") -> str:
        parts = [f"{_CYAN}◆{_RESET} " if self._color else "> "]
        parts.append(self._c(_ACCENT + _BOLD, "social-trends"))
        if context:
            parts += [f" {self._c(_DARK_GRAY, '[')}", self._c(_LIGHT_GRAY, context), self._c(_DARK_GRAY, ']')]
    ...
    def get_prompt_style(self):
        ...
        return Style.from_dict({
            "icon": "#5fdfdf bold",
            "software": "#ff69b4 bold",
            "bracket": "#585858",
            "context": "#bcbcbc",
            "arrow": "#808080",
            "completion-menu.completion": "bg:#303030 #bcbcbc",
            "completion-menu.completion.current": "bg:#ff69b4 #000000",
            "auto-suggest": "#585858",
        })
    ...
    def status(self, label: str, value: str):
        print(f"{self._c(_GRAY, f'  {label}:')}{self._c(_WHITE, f' {value}')}")
    ...
    def help(self, commands: dict):
        ...
        for cmd, desc in commands.items():
            print(f"{self._c(_ACCENT, f'  {cmd:<{max_cmd}}')}  {self._c(_GRAY, desc)}")
        print()

    def print_goodbye(self):
        print(f"\n  {self._c(_GRAY, '◆ Goodbye!')}\n")
    ...
    def get_input(self, pt_session, context: str = "") -> str:
        if pt_session is not None:
            ...
            return pt_session.prompt(FormattedText(self.prompt_tokens(context))).strip()
        return input(self.prompt(context)).strip()
